...
public class Find_Pw_Activity extends FontActivity {
    private ImageButton mBack;
    private EditText mEdit_id;
    private EditText mEdit_pw;
    private EditText mEdit_repw;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mEdit_pw = (EditText)findViewById(R.id.edit_pw);
        mEdit_repw = (EditText)findViewById(R.id.edit_repw);
        mBtn_ok = (Button)findViewById(R.id.btn_ok);
        mBtn_ok.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                String id = mEdit_id.getText().toString();
                String pw = mEdit_pw.getText().toString();
                String repw = mEdit_repw.getText().toString();
                if( !id.equals("") && !pw.equals("") && !repw.equals("") ){
                    if( pw.equals(repw) )
                        new FindPwTask().execute();
                    else
                        Toast.makeText(con,"비밀번호를 다시 확인해주세요",Toast.LENGTH_SHORT).show();
                }else{
                    Toast.makeText(con,"빈칸을 채우세요",Toast.LENGTH_SHORT).show();
                }
            }
        });
    }
    ...
    public InputStream getInputStreamFromUrl(String url) {
        ...
        post.add(new BasicNameValuePair("token", MainActivity.token));
    ...
    public void	getFindFromJSON(){
        String url = "http://dlawogus1.cafe24.com/app/find_pw.php";
        try{
    ...
    private class FindPwTask extends AsyncTask<String, Void, Void> {
        @Override
        ...
        protected void onPostExecute(Void result) {
            ...
            AlertDialog alert_2 = new AlertDialog.Builder(con)
                    .setTitle("비밀번호 변경 오류")
                    .setMessage("등록된 기기가 아닙니다")
                    .setPositiveButton("확인",
                            new AlertDialog.OnClickListener() {
                                public void onClick(DialogInterface dialog, int which) {
                                    dialog.dismiss();
